...
function httpPost(url, body = null, headers = {}) {
  return new Promise((resolve, reject) => {
    const urlObj = new URL(url)
    const options = {
      hostname: urlObj.hostname,
...
async function phase1_SystemHealthCheck() {
  ...
  try {
    ...
    if (dbExists) {
      ...
      try {
        ...
        warn(`Could not query database details: ${e.message}`)
      }
    }
  } catch (e) {
    assert(false, 'Database availability check', e.message)
  }
  ...
  try {
    ...
    // Check for common server errors in response body
    const hasError = res.body.includes('Error') && res.body.includes('500')
    assert(!hasError, 'No 500 errors in server response')
  } catch (e) {
...
async function phase2_AuthenticationTests() {
  ...
  try {
    ...
    assert(res.status === 302 || res.status === 200, 'Admin route responds')
...
async function phase17_SecurityChecks() {
  ...
  try {
    ...
    assert(res.status === 302 || res.status === 401 || res.status === 403,
           'Protected route blocks unauthenticated access')
  } catch (e) {
    assert(false, 'Auth bypass check', e.message)
  }
  ...
  try {
    ...
    assert(throttled || responses.every(r => r.status === 302 || r.status === 200),
