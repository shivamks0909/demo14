...
export default function ExportSuppliersButton({ suppliers }: { suppliers: any[] }) {
    ...
    const handleExport = async (format: 'csv' | 'json') => {
        ...
        try {
            ...
            if (format === 'csv') {
                const headers = ['ID', 'Name', 'Token', 'Status', 'Email', 'Created At']
                const rows = suppliers.map(s => [s.id, s.name, s.supplier_token, s.status, s.contact_email || '', s.created_at])
                ...
                filename = `suppliers-export-${new Date().toISOString().split('T')[0]}.json`
